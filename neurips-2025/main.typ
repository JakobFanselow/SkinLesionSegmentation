...
For training and evaluation, the dataset is divided into training, validation, and test subsets. For this we split the ISIC 2018 training dataset. To achieve a larger training set, all images undergo preprocessing by resizing to a fixed resolution ($512 times 512$), and rotations are applied to enhance model generalization.
